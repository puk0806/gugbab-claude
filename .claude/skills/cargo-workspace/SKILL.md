...
> 주의: 가상 워크스페이스(루트에 `[package]` 없음)에서는 `cargo run`을 루트에서 직접 실행할 수 없다. `-p` 플래그로 크레이트를 지정해야 한다.
...
| `workspace = true` + `version` 동시 지정 | `workspace = true` 사용 시 version 제거. 버전은 루트에서만 관리 |
