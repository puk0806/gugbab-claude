...
React SPA + TypeScript 프론트엔드 프로젝트. 백엔드·게임·학술 스킬 제외.

```
./project-install.sh  →  번호 입력: 2
```
...
| 카테고리 | 종류 | 링크 |
|----------|------|------|
| frontend (76종) | 프레임워크·상태관리·UI·빌드·테스트·성능·SEO·LLM | [→ 목록](../skills/frontend/README.md) |
...
> Memory 훅(memory-pull·memory-sync·memory-stop-guard)은 설치 시 Memory 공유 기능 선택 시 추가됩니다.
...
설치 시 [`examples/CLAUDE.react-spa.md`](../../examples/CLAUDE.react-spa.md)가 대상 프로젝트 루트에 복사됩니다.

이미 CLAUDE.md가 있으면 덮어쓸지 확인 후 처리합니다.

**사전 구성 내용:**
- React SPA 전용 금지 사항 (`any` 타입, `console.log`, prop drilling 3단계 초과)
...
`scripts/gen-settings.js --dev --typescript` 플래그로 생성됩니다.
