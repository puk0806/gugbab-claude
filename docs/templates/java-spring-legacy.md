# 템플릿: java-spring-legacy (5)

Java 11 + Spring Boot 2.5 + WAR + MyBatis 레거시 백엔드 프로젝트.

```
./project-install.sh  →  번호 입력: 5
```
...
| backend — Java 레거시 전용 (5종) | spring-security-5·swagger-springfox-2·redis-redisson-legacy·ehcache-2·aws-sdk-v1 | [→ 목록](../skills/backend/README.md) |
...
> 모던 전용 스킬(spring-security-6·springdoc-openapi-3·redis-redisson-modern·aws-sdk-v2) 제외.
...
설치 시 [`examples/CLAUDE.java-spring-legacy.md`](../../examples/CLAUDE.java-spring-legacy.md)가 대상 프로젝트 루트에 복사됩니다.

이미 CLAUDE.md가 있으면 덮어쓸지 확인 후 처리합니다.

**사전 구성 내용:**
- Java 레거시 전용 금지 사항 (System.out.println, RuntimeException 직접 throw, e.printStackTrace())
