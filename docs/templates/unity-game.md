...
Unity 6 LTS 2D 모바일 게임 개발 프로젝트. 웹 프론트엔드·서버 백엔드·학술 스킬 제외.

```
./project-install.sh  →  번호 입력: 7
```
...
| 카테고리 | 에이전트 | 설명 |
|----------|---------|------|
| game | [game-design-document-writer](../../.claude/agents/game/game-design-document-writer.md) | GDD(게임 디자인 문서) 8섹션 구조로 작성 |
| game | [game-asset-ai-director](../../.claude/agents/game/game-asset-ai-director.md) | AI 에셋 생성 파이프라인 디렉팅 (Midjourney·Leonardo.ai) |
| game | [unity-architect](../../.claude/agents/game/unity-architect.md) | Unity 6 LTS 2D 모바일 아키텍처 설계 |
...
설치 시 [`examples/CLAUDE.unity-game.md`](../../examples/CLAUDE.unity-game.md)가 대상 프로젝트 루트에 복사됩니다.

이미 CLAUDE.md가 있으면 덮어쓸지 확인 후 처리합니다.

**사전 구성 내용:**
- Unity 전용 금지 사항 (`Update()`에 비용 큰 로직 금지, `Find*()` 런타임 루프 호출 금지, `new` 키워드 빈번 생성 금지 — 오브젝트 풀링 사용)
- 공통 원칙 (커밋·푸시 사용자 요청 시에만, 계획 확인 절차)
- 규칙 참조 표 (task-workflow·git·info-verification·agent-design·commands·readme-update·codex-review)
