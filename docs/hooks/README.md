...
| [session-export.js](../../.claude/hooks/session-export.js) | Stop | 세션 대화 요약(요청·응답·수정 파일·Codex 리뷰) 강제 보존 — memory 공유(Y) 모드면 레포 `exports/` 워킹트리에 저장(커밋은 사용자), 아니면 로컬 `~/.claude/projects/<해시>/exports/` (비차단) | ✅ |
