...
- settings.json 생성: `scripts/gen-settings.js` 스크립트로 자동 생성 (2026-06-12 기준)
  - `enabledPlugins`에 `superpowers@superpowers-marketplace: true` 항상 포함 (2026-06-15 fix)
  - `--codex` 플래그 → `codex@openai-codex: true` 추가 (조건부)
  - `--util` 플래그 → 유틸 템플릿용 (verification-guard/skill-md-guard/pending-test-guard 제외)
  - `--dev` 플래그 → dev 템플릿용 (tdd-guard·test-fake-guard·verification-gate·careful-with-judge 포함)
  - `--typescript` → typescript-quality 훅 추가
  - 플래그 없음 → 공통 19종 훅만 (academic·dream-interpretation 등)
  - 회귀 방지: `scripts/gen-settings.test.js` 로 enabledPlugins 검증
